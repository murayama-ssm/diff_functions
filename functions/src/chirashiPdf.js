module.exports = (file, context) => {
  ...
  if (isFront) {
    data = {
      pdf_flyer_front: `https://storage.googleapis.com/chirashi-pdf/${file.name}`,
      pdf_front_period: file.name.slice(0, 8),
    };
  } else {
    data = {
      pdf_flyer_back: `https://storage.googleapis.com/chirashi-pdf/${file.name}`,
      pdf_back_period: file.name.slice(0, 8),
    };
  }
